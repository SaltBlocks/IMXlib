...
nlohmann::json imx_order_details(const char* order_id, CURL* curl = NULL);
nlohmann::json imx_token_details(const char* token_id, CURL* curl = NULL);
...
std::string imx_signable_order_details(const char* nft_address_str, const char* nft_id_str, bool is_offer, nlohmann::json token_details, double price, nlohmann::json fee_data, const char* seller_address_str, CURL* curl = NULL);
